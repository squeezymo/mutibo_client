...
public class SecuredRestBuilder extends RestAdapter.Builder {
    ...
    private class OAuthHandler implements RequestInterceptor {
        ...
        public OAuthHandler(Client client, String tokenIssuingEndpoint, String username,
                            String password, String clientId, String clientSecret, boolean guestAccess) {
            super();
            this.client = client;
            ...
            if (!loggedIn) {
                try {
                    ...
                    try {
                        ...
                        throw new SecuredRestException(e, SecuredRestException.AUTHENTICATION_FAILURE);
                    }

                    // Make sure the server responded with 200 OK
                    if (resp.getStatus() < 200 || resp.getStatus() > 299) {
                        // If not, we probably have bad credentials
                        throw new SecuredRestException("Login failure: " + resp.getStatus() + " - " + resp.getReason(), SecuredRestException.AUTHENTICATION_FAILURE);
                    } else {
                    ...
                    throw new SecuredRestException(e, SecuredRestException.DEFAULT);
                }
            }
            else {
    ...
    public SecuredRestBuilder setGuestAccess(boolean guestAccess) {
        this.guestAccess = guestAccess;
        return this;
    }
    ...
    public RestAdapter build() {
        ...
        if (guestAccess == false && (username == null || password == null)) {
            throw new SecuredRestException("Username or login missing", SecuredRestException.CREDENTIAL_MISSING);
        }

        if (client == null) {
            client = new OkClient();
        }
        OAuthHandler hdlr = new OAuthHandler(client, loginUrl, username, password, clientId, clientSecret, guestAccess);
